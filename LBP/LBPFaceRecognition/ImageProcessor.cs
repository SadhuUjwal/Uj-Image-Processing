...
namespace LBPFaceRecognition
{
    class ImageProcessor
    {
        ...
        public void Train(string path, int r= 1)
        {
            if (string.IsNullOrEmpty(path))
                return;
            ...
            foreach (var item in files)
            {
                if (Path.GetExtension(item).ToLower() != ".csv")
                {
                    ...
                    double[] normalizedLBP = this.NormalizedLBP(testBmp, r);
        ...
        public Image GetMatchedImage(Bitmap testBmp, int r = 1)
        {
            double min = double.MaxValue;
            string mostMatchFile = string.Empty;

            double[] destVal = this.NormalizedLBP(testBmp, r);
            double val;
        ...
        public string GetMatchedImageFile(string dst, int r = 1)
        {
            Bitmap testBmp = new Bitmap(Bitmap.FromFile(dst));
            double min = double.MaxValue;
            string mostMatchFile = string.Empty;
