...
namespace ImageRegistrationBySURF
{
    ...
    public class MyImageProc
    {
        ...
        public static Bitmap DrawCorrespondencesBetweenTwoImages(Bitmap image1, Bitmap image2,
                                       List<Point> Image1Points, List<Point> Image2Points)
        {
            // create a combined bitmap
            int combinedHeight = image1.Height;
            if (image2.Height > combinedHeight)
                combinedHeight = image2.Height;
            Bitmap bmpCombined = new Bitmap(image1.Width + image2.Width, combinedHeight);
            // copy the pixels from each image into the combined bitmap, images are put side by side
            for (int i = 0; i < bmpCombined.Height; i++)
            {
                for (int j = 0; j < bmpCombined.Width; j++)
                {
                    if ((j < image1.Width) && (i < image1.Height))
                    {
                        bmpCombined.SetPixel(j, i, image1.GetPixel(j, i));
                    }
                    else if ((j > image1.Width) && (i < image2.Height))
                        bmpCombined.SetPixel(j, i, image2.GetPixel(j - image1.Width, i));
                    else
                        bmpCombined.SetPixel(j, i, Color.FromArgb(0, 0, 0));
                }
            }

            //--------draw correspondence lines-----------------
            Pen pen = new Pen(Brushes.LightSeaGreen);
            Graphics g = Graphics.FromImage(bmpCombined);

            for (int i = 0; i < Image1Points.Count; i++)
            {
                Point startPt = Image1Points[i];
                Point endPt = new Point(Image2Points[i].X + image1.Width, Image2Points[i].Y);
        ...
        public static Bitmap DrawCorrespondencesBetweenThreeImages(Bitmap image1, Bitmap image2, Bitmap image3,
                                 List<Point> Image1Points, List<Point> Image2Points, List<Point> Image3Points)
        {
            // create a combined bitmap
            int combinedHeight = image1.Height;
            if (image2.Height > combinedHeight)
                combinedHeight = image2.Height;
            if (image3.Height > combinedHeight)
                combinedHeight = image3.Height;
            Bitmap bmpCombined = new Bitmap(image1.Width + image2.Width + image3.Width, combinedHeight);
            // copy the pixels from each image into the combined bitmap, images are put side by side
            for (int i = 0; i < bmpCombined.Height; i++)
            {
                for (int j = 0; j < bmpCombined.Width; j++)
                {
                    ...
                    else if ((j > image1.Width) && (j < image1.Width + image2.Width) && (i < image2.Height))
                        bmpCombined.SetPixel(j, i, image2.GetPixel(j - image1.Width, i));
                    else if ((j > (image1.Width + image2.Width)) && (i < image3.Height))
                        bmpCombined.SetPixel(j, i, image3.GetPixel(j - (image1.Width + image2.Width), i));
                    else
                        bmpCombined.SetPixel(j, i, Color.FromArgb(0, 0, 0));
                }
            }
            ...
            for (int i = 0; i < Image1Points.Count; i++)
            {
                Point startPt = new Point(Image2Points[i].X + image1.Width, Image2Points[i].Y);
                Point endPt = new Point(Image3Points[i].X + image1.Width + image2.Width, Image3Points[i].Y);
                g.DrawLine(pen2, startPt, endPt);
            }
            return bmpCombined;
        }
